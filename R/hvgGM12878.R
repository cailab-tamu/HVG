...
cMatrix <- readMM('../Data/GM12878/matrix.mtx')
colnames(cMatrix) <- readLines('../Data/GM12878/barcodes.tsv')
rownames(cMatrix) <- read.csv('../Data/GM12878/genes.tsv', sep = '\t', stringsAsFactors = FALSE, header = FALSE)[,2]
...
png('../Results/figures/GM12878_Jaccard.png', width = 1500, height = 1500, res = 300)
...
write.csv(outHVG, '../Results/hvgList/GM12878_HVG.csv')
HVG <- names(HVG)
...
write.csv(GO, file = '../Results/annotations/GM12878_GO_UNION.csv')
write.csv(GO2, file = '../Results/annotations/GM12878_GO_INTERSECTION.csv')
...
png('../Results/figures/GM12878.png', width = 4000, height = 3000, res = 300)
...
ggplot(xplot, mapping = aes(x = mean_log, y = cv2_log)) +
  ...
  ggtitle(label = 'LYMPHOBLASTOID CELL LINE (GM12878)', subtitle = toupper(GO2$Description[1]))
dev.off()
