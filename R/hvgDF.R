...
cMatrix <- readMM('hvgDF/matrix.mtx')
rownames(cMatrix) <- read.csv('hvgDF/genes.tsv', header = FALSE, sep = '\t', stringsAsFactors = FALSE)[,2]
colnames(cMatrix) <- readLines('hvgDF/barcodes.tsv')
...
write.csv(cbind(HVG/nBoot), 'DF_HVG.csv')
...
write.csv(GO, file = 'DF_GO_UNION.csv')
write.csv(GO2, file = 'DF_GO_INTERSECTION.csv')
...
library(ggrepel)
library(ggpubr)
png('DF.png', width = 8000, height = 3000, res = 300)
...
p1 <- ggplot(xplot, mapping = aes(x = mean_log, y = cv2_log)) + 
  ...
  ggtitle(label = 'DERMAL FIBROBLAST', subtitle = '(1000 randomizations) Union')
...
p2 <- ggplot(xplot, mapping = aes(x = mean_log, y = cv2_log)) + 
  ...
  xlab(parse(text = 'log[10](Mean)')) +
  ylab(parse(text = 'log[10](CV^2)')) +
  ggtitle(label = 'DERMAL FIBROBLAST',subtitle = '(1000 randomizations) Intersection')
ggarrange(p1,p2)
dev.off()
