...
cMatrix <- readMM('../Data/AT2/SRS2769050.mtx')
rownames(cMatrix) <- readLines('../Data/AT2/genes.tsv')
colnames(cMatrix) <- readLines('../Data/AT2/barcodes.tsv')

# Quality Control
lSize <- colSums(cMatrix)
cMatrix <- cMatrix[,lSize > 1000]
lSize <- colSums(cMatrix)
cMatrix <- cMatrix[,!lSize %in% boxplot.stats(lSize)$out]
...
K <- t(apply(K,1,function(X){as.numeric(rank(X) <= 1000)}))
...
HVG <- pbapply::pbsapply(sCells, function(X){
  ...
  pFit <- predict(fit)
  pVal <- pchisq((cv2/pFit)*1000,999, lower.tail = FALSE)
...
png('../Results/figures/AT2_Jaccard.png', width = 1500, height = 1500, res = 300)
...
write.csv(outHVG, '../Results/hvgList/AT2_HVG.csv')
HVG <- names(HVG)
...
write.csv(GO, file = '../Results/annotations/AT2_GO_UNION.csv')
write.csv(GO2, file = '../Results/annotations/AT2_GO_INTERSECTION.csv')
...
topGO <- unlist(strsplit(GO2$geneID[2], split = '/'))
topGO <- hsa_ENTREZ2SYMBOL(topGO)[,2]

png('../Results/figures/AT2.png', width = 4000, height = 3000, res = 300)
...
ggplot(xplot, mapping = aes(x = mean_log, y = cv2_log)) +
  ...
  ggtitle(label = 'ALVEOLAR TYPE II CELLS', subtitle = toupper(GO2$Description[2]))
dev.off()
