...
cMatrix <- readMM('../Data/KERATINOCYTES/SRS4151081.mtx')
rownames(cMatrix) <- readLines('../Data/KERATINOCYTES/genes.tsv')
colnames(cMatrix) <- readLines('../Data/KERATINOCYTES/barcodes.tsv')
...
png('../Results/figures/KERATINOCYTES_Jaccard.png', width = 1500, height = 1500, res = 300)
...
write.csv(outHVG, '../Results/hvgList/KERATINOCYTES_HVG.csv')
HVG <- names(HVG)
...
write.csv(GO, file = '../Results/annotations/KERATINOCYTES_GO_UNION.csv')
write.csv(GO2, file = '../Results/annotations/KERATINOCYTES_GO_INTERSECTION.csv')
...
png('../Results/figures/KERATINOCYTES.png', width = 4000, height = 3000, res = 300)
...
ggplot(xplot, mapping = aes(x = mean_log, y = cv2_log)) +
  ...
  ggtitle(label = 'KERATINOCYTES', subtitle = toupper(GO2$Description[1]))
dev.off()
