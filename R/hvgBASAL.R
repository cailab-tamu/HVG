...
cMatrix <- readMM('../Data/BASAL/SRS3565197.mtx')
rownames(cMatrix) <- readLines('../Data/BASAL/genes.tsv')
colnames(cMatrix) <- readLines('../Data/BASAL/barcodes.tsv')
...
png('../Results/figures/BASAL_Jaccard.png', width = 1500, height = 1500, res = 300)
...
write.csv(outHVG, '../Results/hvgList/BASAL_HVG.csv')
HVG <- names(HVG)
...
write.csv(GO, file = '../Results/annotations/BASAL_GO_UNION.csv')
write.csv(GO2, file = '../Results/annotations/BASAL_GO_INTERSECTION.csv')
#https://www.ncbi.nlm.nih.gov/pubmed/25217659
...
png('../Results/figures/BASAL.png', width = 4000, height = 3000, res = 300)
...
ggplot(xplot, mapping = aes(x = mean_log, y = cv2_log)) +
  ...
  ggtitle(label = 'BASAL CELLS', subtitle = toupper(GO2$Description[1]))
dev.off()
