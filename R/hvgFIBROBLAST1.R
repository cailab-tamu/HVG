...
cMatrix <- readMM('../Data/FIBROBLAST1/SRS2769050.mtx')
rownames(cMatrix) <- readLines('../Data/FIBROBLAST1/genes.tsv')
colnames(cMatrix) <- readLines('../Data/FIBROBLAST1/barcodes.tsv')

# Quality Control
mtRate <- colSums(cMatrix[grepl('MT-',rownames(cMatrix)),])/colSums(cMatrix)
cMatrix <- cMatrix[,mtRate < 0.1]
cMatrix <- cMatrix[,colSums(cMatrix) > 1000]
cMatrix <- cMatrix[,colSums(cMatrix) < 10000]
...
png('../Results/figures/FIBROBLAST1_Jaccard.png', width = 1500, height = 1500, res = 300)
...
write.csv(outHVG, '../Results/hvgList/FIBROBLAST1_HVG.csv')
HVG <- names(HVG)
...
write.csv(GO, file = '../Results/annotations/FIBROBLAST1_GO_UNION.csv')
write.csv(GO2, file = '../Results/annotations/FIBROBLAST1_GO_INTERSECTION.csv')
#https://www.ncbi.nlm.nih.gov/pubmed/11910357
#https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3831353/
...
png('../Results/figures/FIBROBLAST1.png', width = 4000, height = 3000, res = 300)
...
ggplot(xplot, mapping = aes(x = mean_log, y = cv2_log)) +
  ...
  ggtitle(label = 'FIBROBLAST', subtitle = toupper(GO2$Description[1]))
dev.off()
