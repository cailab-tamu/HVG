...
geneColor <- ifelse(pAdj < 0.01, yes = 'dodgerblue4', no = 'black')
...
legend('topright', legend = c('FDR < 0.01', 'Gamma\nRegression'), bty = 'n', pch = c(16,NA), lty = c(NA,1), pt.cex = 0.5, col = c('dodgerblue4', 'orange'))

regLine <- cbind(log(pMeans),0.98*log(pFit))
regLine <- regLine[order(regLine[,1]),]
points(regLine, type='l', col= 'orange', lwd=2)
par(mar=c(2,3,1,1), mgp = c(1.5,0.5,0))
barplot(as.numeric(GM12878['TMEM9B',]), main = 'TMEM9B', ylab = 'Expression Level (CPM)')
mtext('Cell Index', side = 1, cex = 0.7)
box()
barplot(as.numeric(GM12878['IGKC',]), main = 'IGKC', ylab = 'Expression Level (CPM)')
mtext('Cell Index', side = 1, cex = 0.7)
box()
barplot(as.numeric(GM12878['LTB',]), main = 'LTB', ylab = 'Expression Level (CPM)')
mtext('Cell Index', side = 1, cex = 0.7)
box()
barplot(as.numeric(GM12878['RPL17',]), main = 'RPL17', ylab = 'Expression Level (CPM)')
mtext('Cell Index', side = 1, cex = 0.7)
box()
barplot(as.numeric(GM12878['CCL3',]), main = 'CCL3', ylab = 'Expression Level (CPM)')
mtext('Cell Index', side = 1, cex = 0.7)
box()
barplot(as.numeric(GM12878['FTL',]), main = 'FTL', ylab = 'Expression Level (CPM)')
mtext('Cell Index', side = 1, cex = 0.7)
box()
dev.off()
