...
cMatrix <- readMM('../Data/CHOLANGIOCYTES/SRS3565197.mtx')
rownames(cMatrix) <- readLines('../Data/CHOLANGIOCYTES/genes.tsv')
colnames(cMatrix) <- readLines('../Data/CHOLANGIOCYTES/barcodes.tsv')
...
png('../Results/figures/CHOLANGIOCYTES_Jaccard.png', width = 1500, height = 1500, res = 300)
...
write.csv(outHVG, '../Results/hvgList/CHOLANGIOCYTES_HVG.csv')
HVG <- names(HVG)
...
write.csv(GO, file = '../Results/annotations/CHOLANGIOCYTES_GO_UNION.csv')
write.csv(GO2, file = '../Results/annotations/CHOLANGIOCYTES_GO_INTERSECTION.csv')
...
png('../Results/figures/CHOLANGIOCYTES.png', width = 4000, height = 3000, res = 300)
...
ggplot(xplot, mapping = aes(x = mean_log, y = cv2_log)) +
  ...
  ggtitle(label = 'CHOLANGIOCYTES', subtitle = toupper(GO2$Description[1]))
dev.off()
